...
# 4. FIXED: "Smart Search" Logic & Camera Stability
...
# [LEADER FIX] Force kill any existing camera processes to prevent "Frontend Timeout"
...
# --- OPTIMIZATION ---
HEADLESS_MODE = False     
SKIP_FRAMES = 3           # [FIX] Set to 3 to prevent freezing on Pi CPU
CONF_THRESHOLD = 0.4     
...
try:
   picam2 = Picamera2()
   # [FIX] Lower resolution slightly to ensure stability at 30fps
   ...
   print("TIP: Check ribbon cable or run 'sudo libcamera-hello' to test hardware.")
...
is_auto_mode = False
search_start_time = 0
search_phase = "IDLE" # MOVE, SWEEP, IDLE
...
def send_uart_command(action, duration=0):
   ...
   # If sending manual command (not Auto T or Stop S from loop), disable Auto Mode
   if action not in ['T'] and duration > 0:
       is_auto_mode = False
       search_phase = "IDLE"
       is_manual_override = True
       override_timer = time.time() + (duration if duration > 0 else 2.0)
  
   # Update Status Text based on command
   if action == 'T':
       is_auto_mode = True
       search_phase = "MOVE"
       ...
       # If in auto mode, S means pausing for sweep, don't change status to STOPPED permanently
       if not is_auto_mode: system_status = "STOPPED"
   else:
...
# =======================
def ai_logic_loop():
   global curr_x, curr_y, global_frame_jpg, frame_count, is_manual_override, system_status, last_box, last_box_time, is_auto_mode, search_phase, search_start_time
   ...
   while True:
       try:
           # [ERROR CHECK] If camera times out, we catch and retry
           try:
               frame_rgb = picam2.capture_array()
           except Exception as e:
               print(f"⚠️ Cam IO Error: {e}")
               time.sleep(0.5)
               continue
                   ...
                   if elapsed < 12:
                       ...
                       if search_phase != "MOVE":
                           if ser:
                               with serial_lock:
                                   ser.write(b'T\n')
                                   print("⚡ Auto: Starting Move Phase")
                           search_phase = "MOVE"
                       ...
                       # PHASE 2: SWEEP (12s-20s)
                       if search_phase != "SWEEP":
                           if ser:
                               with serial_lock:
                                   ser.write(b'S\n') # Stop robot
                                   print("⚡ Auto: Starting Sweep Phase")
                           search_phase = "SWEEP"
